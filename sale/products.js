...
function filterSale(products) {
  return products.filter(p => {
    ...
    if (typeof p.tag === "string") {
      return p.tag.toLowerCase() === "sale";
    }

    // tag: ["sale", ...]
    if (Array.isArray(p.tag)) {
      return p.tag.map(t => t.toLowerCase()).includes("sale");
    }
...
async function fetchProducts() {
  ...
  try {
    ...
    // ✅ CHỈ TAG SALE
    filteredProducts = filterSale(allProducts);

    updateCount();

    if (filteredProducts.length === 0) {
      showEmpty("No sale products found.");
      return;
    }
